...
export class AISummaryService {
  ...
  async generateSummary(changeRequest: RemoteConfigChangeRequest): Promise<string> {
    if (!this.openai) {
  ...
  private buildPrompt(changeRequest: RemoteConfigChangeRequest): string {
    ...
    const parameterConditionMapping: string[] = [];
...
${parameterConditionMapping.length > 0 ? `\n=== PARAMETER-CONDITION MAPPING ===\n${parameterConditionMapping.join('\n\n')}` : ''}`;
  ...
  private generateFallbackSummary(changeRequest: RemoteConfigChangeRequest): string {
    const { diff, newConfig } = changeRequest;
    ...
    diff.updatedParams.forEach((paramUpdate) => {
      const details: string[] = [`**${paramUpdate.key}** (Updated)`];
    ...
    diff.addedParams.forEach((paramKey) => {
      const param = newConfig.parameters.find((p) => p.key === paramKey);
      const details: string[] = [`**${paramKey}** (Added)`];
